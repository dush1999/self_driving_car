...
class Dqn():
    ...
    def select_action(self, state):
        with torch.no_grad():  # Disable gradient calculation
            q_values = self.model(state) * 300  # Scale Q-values if necessary
            probs = Fns.softmax(q_values, dim=1)  # Specify the dimension for softmax
            action = probs.multinomial(num_samples=1)  # Specify number of samples to draw
        return action.item()  # Use item() to get the scalar value
    ...
    def learn(self, batch_state, batch_next_state, batch_reward, batch_action):
        ...
        td_loss.backward()  # Remove retain_variables argument
        self.optimizer.step()
